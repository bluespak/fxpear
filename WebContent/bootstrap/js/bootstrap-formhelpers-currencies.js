  ...
  * ====================== */
  ...
  BFHCurrencies.prototype = {
    ...
    , addCurrencies: function () {
      ...
      this.$element.append('<option value="all">All currencies</option>')
      this.$element.append('<hr>')  
      this.$element.append('<option value="USD">USD - United States Dollar</option>')
      this.$element.append('<option value="GBP">GBP - Great British Pound</option>')
      this.$element.append('<hr>')
      for (var currency in this.currencyList) {
   //       this.$element.append('<option value="' + currency + '">' + this.currencyList[currency]['code'] + '</option>')  // don't try to be stupid !!.. refer next line.
          this.$element.append('<option value="' + currency + '">' + currency + ' - ' + this.currencyList[currency]['label'] + '</option>')
      }
    ...
    , addBootstrapCurrencies: function() {
      ...
      for (var currency in this.currencyList) {
        ...
        if (this.options.flags == true) {
          $options.append('<li><a tabindex="-1" href="#" data-option="' + currency +  '"><p style="margin: 0px" class="flag ' + this.flag + '"></p> <small style="visibility: hidden; display: none">' + this.currencyList[currency]['code'] + '</small> ' + this.currencyList[currency]['label'] + '</a></li>')
        } else {
      ...
      if (value) {
        if (this.options.flags == true) {
          $toggle.html('<i class="flag ' + value.substr(0,2) + '"></i> ' + this.currencyList[value]['label'])
        } else {
          $toggle.html(this.currencList[value]['label'])
        }
      }
