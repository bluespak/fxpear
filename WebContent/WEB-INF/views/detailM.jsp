...
<html lang="en">
  <head>
	...
	jQuery.ajax({
	    type: "GET",
	    url: "./summaryM.fx?cuid=me&whoc=S&pseq="+pseq,
	    data: "pseq="+pseq,
...
(function($){
 ...
 function sendDiscuss(){
		if(checkValid_sendDiscuss()){
			...
			document.discussForm.action = "./upDiscussM.fx";
	...
	function completePear(pseq,name,buid){
		...
		if(confirm("Do want to pear up with "+name)){
			location.href = "./completePearM.fx?pseq="+pseq+"&buid="+buid;
		}
	}	
    </script>
   ...
   <c:set var="bcsymbol" scope="request" value="${info.detailinfo.bccd}"/> 
	...
	<div class="pull-left" style="margin-top: 20px"><p class="lead">${info.detailinfo.fname} ${info.detailinfo.lname} wants to exchange ${info.detailinfo.sccd} for ${info.detailinfo.bccd}</p></div>
	  ...
	  <ul class="inline mo" style="background: #fff; font-size: 14px">
	    ...
	    <li><img src="./bootstrap/img/ri.png"></li>
	    <li>${info.detailinfo.sccd}/${info.detailinfo.bccd}</li>
	    <li><img src="./bootstrap/img/ri.png"></li>
	...
	<c:choose>
	 <c:when test="${param.whoc == 'S' && userdiscussresultSize != '0'}">     
        ...
        <ul class="dropdown-menu" role="menu" aria-labelledby="dLabel">
          ...
          <li><a tabindex="-1" href="./detailM.fx?cuid=${list.cuid}&whoc=S&pseq=${info.detailinfo.pseq}"><img src="http://graph.facebook.com/${list.b_fbuid}/picture?width=12&height=12&"> ${list.b_fname} ${list.b_lname}</a></li>
          						</c:when>
								<c:otherwise>
          <li><a tabindex="-1" href="./detailM.fx?cuid=${list.cuid}&whoc=S&pseq=${info.detailinfo.pseq}"><img src="./img/user.png"> ${list.b_fname} ${list.b_lname}</a></li>
								</c:otherwise>
							</c:choose>
				     </c:forEach> 
				    </c:when>
            ...
            <div id="myTabContent" class="tab-content pad" style="background: #fff">
              <div class="tab-pane fade active in" id="home">
                <ul class="unstyled mo">
                  ...
                  <li class="well text-center" style="margin: 20px"><a id="s_Rate" type="text" style="color: #2b2b2b; text-decoration: none"></a> ${info.detailinfo.sccd} = <a id="b_Rate" type="text" style="color: #2b2b2b; text-decoration: none"></a> ${info.detailinfo.bccd}<BR>Sell: <a id="Bid" type="text"  style="color: #2b2b2b; text-decoration: none"></a>&nbsp;&nbsp;Buy: <a id="Ask" type="text"  style="color: #2b2b2b; text-decoration: none"></a><br><small>(Source: Yahoo Finance)<br><a id="date" type="text"  style="color: #2b2b2b; text-decoration: none"></a> <a id="time" type="text"  style="color: #2b2b2b; text-decoration: none"></a></small></li>
